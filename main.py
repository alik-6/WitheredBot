...
async def help(ctx):
    ...
    for key in bot.walk_commands():
        if str(key) not in excluded:
            help_embed(name=f"{PREFIX}{key}", value="`cmd`")
    ...
    for file in listdir("./plugins"):
            ...
            if file.endswith("_plugin.py"):
                try:
                    ...
                    aprint(f"Plugin \"{data['name']}\" Failed!")
                finally:
                    pass
...
def run_bot():
    with open('token.json', 'r') as token:
        try:
            ...
            aprint("It looks like discord server are having some issues,please try again later status: https://discordstatus.com/")
        except errors.ConnectionClosed:
            aprint("Discord Unexpectidly Closed the connection")
        finally:
            pass
